...
<form action="${pageContext.request.contextPath }/student/studentwritegrade/add.action" class="form-horizontal">
	...
	<div class="row">
                ...
                <div class="col-sm-6">
                		<select class="form-control input-sm" name="studentId" >
                        	...
                        	<c:forEach items="${studentInfolist}" var="studentInfo">
                            	<option value="${studentInfo.studentId}" ${studentInfo.studentId==studentWriteGrade.studentId?'selected':''}>${studentInfo.studentName}</option>
                            </c:forEach>
                        </select>
                </div>
            </div>
      ...
      <div class="row">
           <div class="col-sm-5">
            <div class="form-group">
            		<label class="col-sm-3 control-label">监考和阅卷人</label>
                <div class="col-sm-6">
                		<select class="form-control input-sm" name="staffId" >
                        	<option>请选择</option>
                        	<c:forEach items="${staffInfolist}" var="staffInfo">
                            	<option value="${staffInfo.staffId}" ${staffInfo.staffId==studentWriteGrade.staffId?'selected':''}>${staffInfo.staffName}</option>
                            </c:forEach>
                        </select>
                </div>
            </div>
    ...
    <div class="row">
    	<div class="col-sm-5">
        	<div class="form-group">
                ...
                <div class="col-sm-9">
               			<input type="text" name="writeGradeTime" value="<fmt:formatDate value="${studentWriteGrade.writeGradeTime}" type="both" pattern="yyyy-MM-dd"/>" onclick="WdatePicker()" readonly="readonly" class="form-control input-sm" placeholder="请输入考试时间"/>
                </div>
            </div>
        </div>
